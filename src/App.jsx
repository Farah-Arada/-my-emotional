...
import { Smile, Home, Volume2, AlertCircle, Loader2, Hand, Heart, Palette, Trash2 } from 'lucide-react';
...
export default function App() {
  ...
  const [currentMode, setCurrentMode] = useState('feelings'); // 'feelings' | 'needs' | 'draw'
  ...
  const [strokeColor, setStrokeColor] = useState('#8b5cf6'); // اللون الافتراضي (بنفسجي)
  ...
  const handleStart = useCallback(async (e) => {
    ...
    setStep('welcome');
    setCurrentMode('feelings'); // دائماً نبدأ بالمشاعر
  ...
  const clearCanvas = () => {
    ...
    // إعادة تعبئة الخلفية باللون الأبيض
    ctx.fillStyle = '#ffffff';
    ctx.fillRect(0, 0, canvas.width, canvas.height);
  };

  useEffect(() => {
    // تهيئة السبورة بخلفية بيضاء عند فتح قسم الرسم
  ...
  // 🎤 تشغيل السؤال عند دخول التطبيق أو تغيير القسم
  useEffect(() => {
    if (step === 'app' && childName && childGender) {
      let qText = '';
      if (currentMode === 'feelings') {
        qText = childGender === 'boy' 
          ? `كيف تشعر اليوم يا ${childName}؟` 
          : `كيف تشعرين اليوم يا ${childName}؟`;
      } else if (currentMode === 'needs') {
        qText = childGender === 'boy' 
          ? ` أخبرني ماذا تريد يا ${childName}؟` 
          : ` أخبريني ماذا تريدين يا ${childName}؟`;
      } else if (currentMode === 'draw') {
        qText = childGender === 'boy' 
          ? `ارسم لي كيف تشعر يا ${childName}` 
          : `ارسمي لي كيف تشعرين يا ${childName}`;
      }
  ...
  }, [step, currentMode, childName, childGender, playDynamicAudio]);
