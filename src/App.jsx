...
import { Smile, Home, Volume2, AlertCircle, Play } from 'lucide-react';
...
export default function App() {
  ...
  const [audioEnabled, setAudioEnabled] = useState(false); // ← جديد: التأكد من تفعيل الصوت
  ...
  // ─────────────────────────────────────────────────
  // 🔓 تفعيل الصوت يدوياً (مطلوب للموبايل)
  // ─────────────────────────────────────────────────
  ...
  // ─────────────────────────────────────────────────
  useEffect(() => {
    if (step === 'app' && childName && childGender && audioEnabled) {
  ...
  }, [step, childName, childGender, audioEnabled, playDynamicAudio]);
    ...
    <div className="min-h-screen relative text-right overflow-x-hidden select-none" dir="rtl" style={{ fontFamily: "'Cairo', sans-serif" }}>
      ...
      {step === 'welcome' && (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-yellow-300 via-pink-400 to-purple-400 text-center relative overflow-hidden">
          ...
          {/* ← جديد: زر تفعيل الصوت للموبايل */}
          {!audioEnabled && (
            <button
              onClick={enableAudio}
              className="absolute z-50 bg-white/90 p-6 rounded-3xl shadow-2xl animate-bounce flex flex-col items-center gap-3"
            >
              <Play size={48} className="text-purple-600" />
              <span className="font-black text-purple-800 text-xl">اضغطي هون لتفعيل الصوت 🔊</span>
            </button>
          )}
